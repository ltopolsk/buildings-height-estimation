...
def get_transforms(height=512, width=512):
    ...
    ], additional_targets={'dsm': 'image', 'sar': 'image'})
    ...
    # Radar has "speckle" noise. MultiplicativeNoise mimics this.
    sar_transform = A.Compose([
        A.MultiplicativeNoise(multiplier=(0.9, 1.1), elementwise=True, p=0.5)
    ])
    
    return spatial_transform, pixel_transform, sar_transform
